...
static void inspect_wal(const std::filesystem::path& wal_path)
{
  ...
  while (in.peek() != EOF)
  {
    ...
    if (hdr.lr_length > sizeof(smoldb::LogRecordHeader))
    {
      in.seekg(hdr.lr_length - sizeof(smoldb::LogRecordHeader), std::ios::cur);
    }
  }
...
class AriesTest : public ::testing::Test
{
 protected:
  ...
  void crash(std::unique_ptr<smoldb::SmolDB>& db) { db.reset(); }

  std::unique_ptr<smoldb::SmolDB> restart(size_t buffer_pool_size = 16)
  {
    smoldb::DBConfig config{test_dir_, buffer_pool_size};
    auto db =
        std::make_unique<smoldb::SmolDB>(config, io_context_.get_executor());
    db->startup();
    return db;
  }

  smoldb::Schema make_simple_schema()
  {
...
{
  smoldb::RID rid;
  smoldb::Schema schema = make_simple_schema();
  ...
  {
    auto db = restart();
    smoldb::Table<>* table = db->get_table("users");
...
TEST_F(AriesTest, CrashWithInFlightLoserTransaction)
{
  ...
  // Phase 1: Setup initial state
  {
    ...
    db->create_table(1, "users", schema);
    smoldb::Table<>* table = db->get_table("users");
    smoldb::TransactionID setup_txn = db->begin_transaction();
    smoldb::Row initial_row(schema);
  ...
  // Phase 2: Start a loser transaction and crash
  {
    ...
    smoldb::Table<>* table = db->get_table("users");
    ASSERT_NE(table, nullptr);
    smoldb::TransactionID loser_txn = db->begin_transaction();
    smoldb::Row new_row(schema);
...
TEST_F(AriesTest, CrashDuringUndoRecoversCorrectly)
{
  ...
  */
  ...
  // Phase 3: Restart but inject a crash during the undo phase.
  {
    ...
    ASSERT_THROW(
        db->startup_with_crash_point(smoldb::RecoveryCrashPoint::DURING_UNDO),
        std::runtime_error);
    crash(db);
  }
...
TEST_F(AriesTest, CrashAfterRuntimeAbort)
{
  ...
  // Then, crash before the final ABORT record is guaranteed to be flushed.
  {
    auto db = restart();
    smoldb::Table<>* table = db->get_table("users");
    smoldb::TransactionID txn_to_abort = db->begin_transaction();
    smoldb::Row new_row(schema);
...
TEST_F(AriesTest, MultipleLoserTransactions)
{
  ...
  // Phase 1: Setup initial state with two rows.
  {
    ...
    smoldb::Table<>* table = db->get_table("users");
    smoldb::TransactionID setup_txn = db->begin_transaction();
    smoldb::Row row1(schema), row2(schema);
  ...
  // crash.
  {
    ...
    smoldb::TransactionID loser1 = db->begin_transaction();
    smoldb::TransactionID loser2 = db->begin_transaction();

    smoldb::Row new_row1(schema), new_row2(schema);
...
TEST_F(AriesTest, UpdateSameRowMultipleTimesAndAbort)
{
  ...
  // Phase 2: One transaction updates the same row twice, then crashes.
  {
    ...
    smoldb::Table<>* table = db->get_table("users");
    smoldb::TransactionID loser_txn = db->begin_transaction();
    smoldb::Row row_v2(schema), row_v3(schema);
